...
public class Magic: NSObject {
   ...
   public var rpcProvider: RpcProvider
    ...
    ///   - authOnlyViewProvider: An optional `UIViewController` provider for apps utlizing Magic for Authentication only (not advisable for Web3 based apps)
    ...
    public convenience init(apiKey: String, authOnlyViewProvider: MagicAuthOnlyViewHostProviding? = nil, locale: String = Locale.current.identifier) {
        self.init(urlBuilder: URLBuilder(apiKey: apiKey, locale: locale))
    }

    /// Core constructor
    private init(urlBuilder: URLBuilder, authOnlyViewProvider: MagicAuthOnlyViewHostProviding?) {
         self.rpcProvider = RpcProvider(urlBuilder: urlBuilder, viewHostProvider: authOnlyViewProvider)
